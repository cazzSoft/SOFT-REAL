...
//mostrat la modal de acabado_cubierta Create
   function btnMostrarCreateCubiertaEs(){
    //$('#formAcabado')[0].reset();//limpiar formulario
    $('#name').val("");
    $('#indice').val("");
    $('#headerEstructura').html('<span class="fa fa-list"></span> CREAR ESTRUCTURA CUBIERTA');
...
//mostrat la modal de acabado_closet Edit
   function btnMostraEditrCubiertaEs(id){

     $("#headerEstructura").html('<i class="fa fa-edit"></i> ACTUALIZAR ESTRUCTURA CUBIERTA');
     ...
     $("#formEstructura").attr('action','cubierta/'+id);//fijamos laruta para editar window.location.protocol+'//'+window.location.host+'/closet/'+id);
