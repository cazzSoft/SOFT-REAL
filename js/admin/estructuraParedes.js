...
//mostrat la modal de acabado_cubierta Create
   function btnMostrarCreateParedes(){
    //$('#formAcabado')[0].reset();//limpiar formulario
    $('#name').val("");
    $('#indice').val("");
    $('#headerEstructura').html('<span class="fa fa-list"></span> CREAR ESTRUCTURA PARED');
...
//mostrat la modal de acabado_closet Edit
   function btnMostraEditrParedes(id){

     $("#headerEstructura").html('<i class="fa fa-edit"></i> ACTUALIZAR ESTRUCTURA PARED');
      $('#formEstructura')[0].reset();//limpiar formulario
     $.get('paredes/'+id+'/edit', function (data) {
         $('#name').val(data.name);
     ...
     $("#formEstructura").attr('action','paredes/'+id);//fijamos laruta para editar window.location.protocol+'//'+window.location.host+'/closet/'+id);
